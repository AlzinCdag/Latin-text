...
this method uses a cast. checks three levels.*/
    ...
    public Room getRoom() {
   // Room r = null;
  /*  if (getLocation().getLocation() == Objects.Rooms) // the object's location is a room, because its location is the object Rooms, which only rooms can be in directly
        r = (Room) getLocation();
    else if (getLocation().getLocation().getLocation() == Objects.Rooms)
        r = (Room) getLocation().getLocation();
    else {System.out.println("the " + toString() + " doesn't appear to be in a room or a container that is directly contained by a room.");
    r = (Room) getLocation();
    Exception E = new Exception("None of the options in getRoom worked: neither this object's location nor its' location's location were Rooms.");
            throw E;}
    */
 /* Class<? extends Room> c = Objects.Forge.getClass();
  for( Noun n = this; n.getClass() != c; n = n.getNounLocation() )
          {System.out.println("n = " + n);
              if (n.getNounLocation() == null) //for example, the Rooms object
          {    r = null;
          n = Syntax.getCurrentPlayerLoc();} // prevent null pointers
  ...
  */
   ...
   @Override
    ...
    public void look(int level) {
      System.out.println(" is the level. The Location is ");
    /*    String tabs = " ";
     if (level != -1)
     {
       if (level != 0) {
       ...
       if(mLocation == Objects.Rooms)
       {System.out.println(mShortDescription);
       System.out.println( tabs + getLongDescription());
       for (Noun n : mContained)
    { if (!n.getInvisible() && (n != Objects.Player))
    {   System.out.println("    hic est " + n.getLongDescription());
        if (!n.mContained.isEmpty())
            {
            for (Noun m:mContained)
                {m.look(level + 2);}
            }// if the thing in the room has something in it, we can commence the usual container look process
    }//if not invisible, not the player
    }//for
