...
/**
 *The main class which splits the input so that the code can read it.
...
public static LatinVerbs LVerbs = new LatinVerbs();
 ...
 public static void setCurrentPlayerLoc(Location loc) {currentPlayerLoc = loc; }
   ...
   static ArrayList<SingleSyntax> TalkSyntax = new ArrayList<SingleSyntax>();
   static HashMap<SingleSyntax, LatNoun> TalkMap= new HashMap<SingleSyntax,LatNoun>();
   ...
   public static void fillSyntaxList() {
      ...
      justVerbSyntaxes.add(new SingleSyntax("n",Verbs.N,false));
       ...
       SingleSyntax rexSyntax = new SingleSyntax("rex(.*)", Verbs.TALKTO, false);
       TalkSyntax.add(rexSyntax);
       TalkMap.put(rexSyntax, Objects.Rex);
      
       SingleSyntax clemSyntax = new SingleSyntax("clemens(.*)", Verbs.TALKTO, false);
       TalkSyntax.add(clemSyntax);
       TalkMap.put(clemSyntax, Objects.Clemens);
       
              SingleSyntax mercSyntax = new SingleSyntax("mercurie(.*)", Verbs.TALKTO, false);
       TalkSyntax.add(mercSyntax);
       TalkMap.put(mercSyntax, Objects.Mercury);
       justVerbSyntaxes.add(new SingleSyntax ("mercurius(.*)",Verbs.CORRECTMERC, false));
  ...
  boolean matched = false;
    ...
    boolean isTalkSyntax = false;
    
     for ( SingleSyntax talk : TalkSyntax)
    { String sentence = s.findInLine(talk.getString());
    
        if (sentence != null )
        {isTalkSyntax = true;
        matched = true;
        String subsentence = s.match().group(1);
       
        Objects.Winner = TalkMap.get(talk);
      //  System.out.println("Winner = " + Objects.Winner.mShortDescription + " string: " + subsentence);
        Syntax.match(subsentence);
    ...
    {if (odd.getString().equalsIgnoreCase(x) )
        ...
        if (Objects.Winner.handle() == false) {
...
//if you need to assemble the sentence from scratch
     if (!matched) {
         ...
         if (  ((cTest.contains("DP") || cTest.contains("DS"))&& ((V != null) && (V.getGoesWithDative()))) || ((cTest.contains("OP") || (cTest.contains("OS"))&& (V != null) && (V.getGoesWithAblative())) && (!words[i].equalsIgnoreCase("me"))) ||(cTest.contains("OS") && (!words[i].equalsIgnoreCase("te"))))
                 {
                   if ((Objects.parse(words[i]).getCharacter() && (cTest.contains("DP") || cTest.contains("DS"))) || (!Objects.parse(words[i]).getCharacter() && (cTest.contains("OP") || cTest.contains("OS"))) )
        ...
        if (IO.handle() == false) {
        if (DO.handle() == false){
        if ((IO.getCharacter() &&(ioword.contains("DP") || ioword.contains("DS"))) || (!IO.getCharacter() &&(ioword.contains("OP") || ioword.contains("OS")))) {
          //  System.out.println("character: " + IO.getCharacter() + " ioword: " + ioword );
            if (V.handle() == false)
        System.out.println("I didn't understand that sentence. V.handle = false, ");
        }
        //else
           // System.out.println("false: character: " + IO.getCharacter() + " ioword: " + ioword);
        }//DO
        }//IO
         }//IO is not null
